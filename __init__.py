...
import os , time , ftplib , sys , pywifi , json
...
def load_config ():
    ...
    try:
        ...
        print ("WARNING: The config file in 'boot' read failed, pass now.")
load_config ()
# ============== 配置读取 & 网络连接 ============================
...
def videodecoder ():
    while True:
        try:
            ...
            print ('ERROE : Videocoder thread work failed. Restarting .')
        ...
        try:
            ...
            for i_dir in os.listdir ('video'): # 遍历 video 目录下的日期目录
                ...
                if os.path.isdir ('video/' + i_dir):
                    # 如果是日期文件夹 , 则进入
                    for i_file in os.listdir ('video/' + i):
                        # i [:-4] 去掉后缀
                        createTime = time.mktime(time.strptime (i,'%Y-%m-%d-%a-%H-%M-%S')) 
                        # ↑ 此方法将时间转为秒
            ...
            print ('ERROR : Remove thread worked failed. Restart now.')
